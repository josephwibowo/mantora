...
from mantora.connectors import DatabricksAdapter


def _golden_dir() -> Path:
    return Path(__file__).parent / "golden" / "databricks" / "traces"
...
def test_databricks_adapter_normalizes_golden_traces(trace_path: Path) -> None:
...
def test_databricks_adapter_categorizes_core_tools() -> None:
    adapter = DatabricksAdapter()
    ...
    assert adapter.categorize_tool("list_catalogs") == "list"
